...
def pcl_callback(pcl_msg):
    ...
    tem_pcl_statistical_pub.publish(ros_cloud_filtered)
    ...
    # Publish
    ros_cloud_filtered = pcl_to_ros(cloud_filtered)
    tem_pcl_voxel_pub.publish(ros_cloud_filtered)
    ...
    # Publish
    ros_cloud_filtered = pcl_to_ros(cloud_filtered)
    tem_pcl_passthrough_pub.publish(ros_cloud_filtered)
...
def pr2_mover(object_list):
    ...
    # (I did not use this step to map the side tables. Will come back later when have more time.)

    # Loop through the pick list
    dict_list = []
    for ii in range(len(object_names)):
        object_name.data = object_names[ii]

        # Get the PointCloud for a given object, obtain it's centroid and create pick_pose
        for obj in object_list:
            if object_names[ii] == obj.label:
...
if __name__ == '__main__':
    ...
    # Temperory Publishers for writeup
    tem_pcl_statistical_pub = rospy.Publisher('/tem_pcl_statistical',PointCloud2,queue_size=1)
    tem_pcl_voxel_pub = rospy.Publisher('/tem_pcl_voxel',PointCloud2,queue_size=1)
    tem_pcl_passthrough_pub = rospy.Publisher('/tem_pcl_passthrough',PointCloud2,queue_size=1)
